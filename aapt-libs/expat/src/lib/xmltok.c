...
{
  int ret = ((p[0] & 0xE0) == 0xC0) && ((p[1] & 0xC0) == 0x80);
  printf("___utf8_isNmstrt2____RET=%d, utf8byte:0X%02X%02X\n", ret,
         (unsigned char)(p[0]), (unsigned char)(p[1]));
  return ret; //UTF8_GET_NAMING2(nmstrtPages, (const unsigned char *)p);
}
...
{
  int ret = ((p[0] & 0xF0) == 0xE0) && ((p[1] & 0xC0) == 0x80) && ((p[2] & 0xC0) == 0x80);
  printf("___utf8_isNmstrt3____RET=%d, utf8byte:0X%02X%02X%02X\n", ret,
         (unsigned char)(p[0]), (unsigned char)(p[1]), (unsigned char)(p[2]));
  return ret; //UTF8_GET_NAMING3(nmstrtPages, (const unsigned char *)p);
}
